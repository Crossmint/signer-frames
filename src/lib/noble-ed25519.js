...
const arange = (n, min, max = MASK, msg = 'bad number: out of range') =>
...
const wNAF = n => {
  ...
  for (let w = 0; w < pwindows; w++) {
    ...
    const offF = off,
      offP = off + Math.abs(wbits) - 1; // offsets, evaluate both
    const isEven = w % 2 !== 0,
      isNeg = wbits < 0; // conditions, evaluate both
    if (wbits === 0) {
...
export { getPublicKey, getPublicKeyAsync, sign, signAsync, verify, verifyAsync, Point };
