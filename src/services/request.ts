...
> {
  ...
  private endpoint: (input: I, authData: { jwt: string; apiKey: string }) => string;
  private method: string;
  private encrypted: boolean;
  private encryptionService: EncryptionService;
  private getHeaders: (authData: { jwt: string; apiKey: string }) => Record<string, string>;
  ...
  async execute(input: I, authData: { jwt: string; apiKey: string }): Promise<O> {
    ...
    if (this.encrypted) {
      const parsedResponseData = this.encryptedPayloadSchema.parse(json);
      json = this.encryptionService.decryptBase64(
