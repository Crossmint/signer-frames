...
import type { Base58Utils } from './utils';
...
export class Ed25519Service {
  constructor(private readonly base58Utils: Base58Utils) {}
  /**
    ...
    try {
      const keyBytes =
        typeof privateKeyBase58orArray === 'string'
          ? this.base58Utils.base58Decode(privateKeyBase58orArray)
  ...
  getSecretKey(
    ...
    const publicKeyBytes =
      typeof publicKeyBase58orArray === 'string'
        ? this.base58Utils.base58Decode(publicKeyBase58orArray)
   ...
   */
  async sign(message: Uint8Array | string, privateKeyBase58: Uint8Array | string): Promise<string> {
    try {
      const messageBytes =
        typeof message === 'string' ? new TextEncoder().encode(message) : message;

      const keyBytes =
        typeof privateKeyBase58 === 'string'
          ? this.base58Utils.base58Decode(privateKeyBase58)
          : privateKeyBase58;
      ...
      return this.base58Utils.base58Encode(signatureBytes);
    } catch (error) {
  ...
  async verifySignature(
    ...
    try {
      ...
      const signatureBytes = this.base58Utils.base58Decode(signatureBase58);
      const publicKeyBytes = this.base58Utils.base58Decode(publicKeyBase58);
