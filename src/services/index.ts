...
import { UserMasterKeyManager } from './user/key-manager';
...
export type CrossmintFrameServices = {
  ...
  userKeyManager: UserMasterKeyManager;
...
export const createCrossmintFrameServices = () => {
  ...
  const encryptionKeyProvider = new EncryptionKeyProvider(storageService);
  const teeKeyService = new TEEKeyProvider();
  const encryptionService = new EncryptionService(encryptionKeyProvider, teeKeyService);
  ...
  const userKeyManager = new UserMasterKeyManager(
    crossmintApiService,
