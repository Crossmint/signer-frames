...
const init = () => {
  ...
  const XMIF = {
    ...
    /**
     * Initialize all default event handlers
     * This sets up the default implementations for all predefined events
     */
    initEventHandlers() {
      // Register default handlers for all predefined events
      for (const eventName in events.handlers) {
        // Only register if no custom handler has been set
        if (events.handlers[eventName] != null) {
          events.registerHandler(eventName, async (data) => {
            console.log(`Default handler for ${eventName}`, data);
            return { success: false, message: "No implementation provided" };
          });
        }
      }
    },
