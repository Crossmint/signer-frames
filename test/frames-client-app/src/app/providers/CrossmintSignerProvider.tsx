...
const InitializingComponent = () => (
	<div className="flex flex-col items-center justify-center p-4">
		...
		<p className="text-cm-text-secondary text-center">
...
const CrossmintSignerContext = createContext<CrossmintSignerContext | null>(
	null,
);
...
export function useCrossmintSigner() {
	...
	if (context == null) {
		throw new Error(
			"useCrossmintSigner must be used within a CrossmintSignerProvider",
		);
	}
	return context;
}
...
export default function CrossmintSignerProvider({
	children,
	iframeUrl = new URL(
		process.env?.NEXT_PUBLIC_SECURE_ENDPOINT_URL ?? "secure.crossmint.com",
	),
}: CrossmintSignerProviderProps) {
	const [otpDialogOpen, setOtpDialogOpen] = useState(false);
	const [isInitializingSigner, setIsInitializingSigner] = useState(false);
	const [solanaSigner, setSolanaSigner] =
		useState<CrossmintSolanaSigner | null>(null);
		...
		return {
			...
			signTransaction: async (
				...
				try {
					...
					console.log(
						`Transaction serialized: ${serializedTx.substring(0, 20)}...`,
					);
	...
	const handleCreateSignerEvent = async () => {
		...
		if (iframeWindow.current == null || jwt == null || apiKey == null) {
			throw new Error(
				"Failed to create signer. The component has not been initialized",
			);
		}
	...
	const handleEncryptedOtpEvent = async (
		encryptedOtp: string,
		chainLayer: "solana",
	) => {
		assertInitialized();
